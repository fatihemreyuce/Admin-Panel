...
import { useBlogById, useUpdateBlog } from "@/hooks/use-blog";
...
export default function BlogEditPage() {
	...
	const { data: blogData, isLoading: isLoadingBlog, error: blogError } = useBlogById(blogId);
	...
	// Blog verilerini form'a yükle
	useEffect(() => {
		if (blogData) {
			setFormData({
				slug: blogData.slug || "",
				categoryId: blogData.category?.id || 0,
				tags: blogData.tags || [],
				status: blogData.status || "DRAFT",
				translations: blogData.translations && blogData.translations.length > 0 
					? blogData.translations.map(translation => ({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			await updateBlogMutation.mutateAsync(apiData);
			navigate("/blogs");
		} catch (error: any) {
	...
	const handleTagToggle = (tag: any) => {
		...
		if (isSelected) {
			const newTags = formData.tags.filter((t) => t.id !== tag.id);
	...
	const isFormDisabled = updateBlogMutation.isPending || isLoadingBlog;
					...
					<CardContent className="pt-6">
						<div className="text-center">
							...
							<p className="text-sm text-gray-500 mt-2">
								{blogError instanceof Error ? blogError.message : "Bilinmeyen hata"}
							</p>
							<Button
