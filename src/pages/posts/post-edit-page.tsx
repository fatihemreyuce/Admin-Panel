...
export default function PostEditPage() {
    ...
    const { data: post, isLoading } = usePostById(Number(id));
    const updatePostMutation = useUpdatePost(Number(id));
    ...
    useEffect(() => {
        if (post) {
            reset({
                ...
                status: "PUBLISHED" // Default to published since we're editing existing post
            });
    ...
    const onSubmit = async (data: any) => {
        ...
        if (!selectedImage && !post?.featuredImage) {
        ...
        // Create a dummy file if no new image is selected but we have an existing image
        const imageFile = selectedImage || new File([], 'existing-image.jpg', { type: 'image/jpeg' });
        ...
        try {
            await updatePostMutation.mutateAsync(postRequest);
            navigate(`/posts/${id}`);
        } catch (error) {
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-gray-600">"{post.title}" postunu düzenleyin</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    {post.featuredImage && (
                                        <div className="flex items-center gap-2">
                                            ...
                                            />
                                            <span className="text-sm text-gray-600">Mevcut görsel</span>
                                        </div>
                                    )}
                                    <Input
                                        type="file"
                                        ...
                                        placeholder="Yeni görsel seç (opsiyonel)"
                                    />
                                    {selectedImage && (
                                        <span className="text-sm text-green-600">
                                            Yeni görsel: {selectedImage.name}
                                        </span>
                                    )}
                                </div>
                            </div>
                ...
                <div className="flex gap-4 justify-end">
                    ...
                    <Button type="submit" disabled={updatePostMutation.isPending}>
                        {updatePostMutation.isPending ? "Güncelleniyor..." : "Post Güncelle"}
                    </Button>
                </div>
            </form>
