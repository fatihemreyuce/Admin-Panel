...
export default function PostEditPage() {
	...
	useEffect(() => {
		if (postData) {
			setFormData({
				...
				tags: postData.tags || [],
				status: "PUBLISHED", // API'den gelen status field'ı yok, varsayılan olarak PUBLISHED
				translations: [
					{
						languageCode: "tr",
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!formData.slug || !formData.categoryId) {
			alert("Lütfen tüm gerekli alanları doldurun");
			return;
		}

		try {
			await updatePostMutation.mutateAsync(formData as PostRequest);
			navigate("/posts");
		} catch (error) {
				...
				<form onSubmit={handleSubmit} className="space-y-6">
								...
								<CardContent className="space-y-4 pt-6">
									...
									<div>
										...
										<div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
											...
											{postData.featuredImage && !formData.image && (
												<div className="mt-2 text-sm text-gray-600">
													Mevcut görsel korunacak
												</div>
											)}
										</div>
									</div>
					...
					<div className="flex justify-end space-x-4">
						...
						>
							...
							<span>{updatePostMutation.isPending ? "Güncelleniyor..." : "Güncelle"}</span>
						</Button>
					</div>
				</form>
			</div>
